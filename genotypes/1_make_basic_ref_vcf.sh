...
## We may need to fix this, to only use the samples we need
## removing covid-19 samples from the vcf file. 
...
bcftools view -S <(tail -n +2 Covid19GenotypeFixes.txt | cut -f2) -v snps -m2 -M2 -i 'INFO/MAF>0' mergeAll.2021-07-29/2021-07-29_merge.vcf.gz --threads 4 \
  | bcftools reheader -s <(tail -n +2 Covid19GenotypeFixes.txt | cut -f5) --threads 4 \
